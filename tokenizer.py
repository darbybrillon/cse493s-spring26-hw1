...
class tokenizer:
    ...
    def encoding(self, input: str) -> list[int]:
        encoded_input = [self.vocab_index_mapping["<s>"]] + [self.vocab_index_mapping.get(token, self.vocab_index_mapping["<UNK>"]) for token in input.split()] + [self.vocab_index_mapping["<\s>"]]
